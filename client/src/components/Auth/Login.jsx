...
export default function Login(props){
    ...
    const handleGoogleLogin = () => {
        let timer= null;
        const googleLoginURL = "http://143.198.233.198:5000/api/login/google";
        ...
        if (newWindow) {
          timer = setInterval(() => {
            if (newWindow.closed) {
              ...
              axios.get("http://143.198.233.198:5000/api/auth/user", { withCredentials: true })
              .then((res) => {
    ...
    const handleLogin = (e) => {
        e.preventDefault();
        axios.post("http://143.198.233.198:5000/api/login", input, {
        withCredentials: true
        }).then((res) => {
        if (res.data === "Successfully Authenticated") {
            axios.get("http://143.198.233.198:5000/api/auth/user", { withCredentials: true }).then((res) => {
                console.log(res.data)
                localStorage.setItem('id', res.data.id);
                localStorage.setItem('fullName', res.data.fullName);
                localStorage.setItem('email', res.data.email);
